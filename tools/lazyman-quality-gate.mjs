...
function maxRepetition(dump) {
  ...
  slides.forEach((s) => {
    ...
    const tokens = texts.toLowerCase().match(/[\u4e00-\u9fa5]{2,}|[a-z0-9_]+/g) || [];
    const freq = {};
    tokens.forEach((t) => {
      freq[t] = (freq[t] || 0) + 1;
    });
...
function titleLengthVariance(dump) {
  const slides = dump?.slides || [];
  const lengths = slides.map((s) => (s.title || '').trim().length).filter((l) => l > 0);
...
function countHalfFilledSlides(dump) {
  ...
  const halfFilled = contentSlides.filter((s) => {
    const textCount = (s.texts || []).filter((t) => (t || '').trim().length > 0).length;
    return textCount <= 2;
  }).length;
...
export function lazymanQualityGate(result) {
  ...
  const checks = {
    autoScore: (result.autoScore || 0) >= 90,
    systemScore: (result.qualityScore?.overall || 0) >= 92,
    noBlockers: (result.blockerCount || 0) === 0,
    noPageNumbers: !hasPageNumbers(dump),
    noMetadata: !hasMetadata(dump),
    lowRepetition: maxRepetition(dump) <= 3,
    titleVarianceOK: titleLengthVariance(dump) <= 50
  };

  const halfFilledInfo = countHalfFilledSlides(dump);
  checks.lowHalfFilled = halfFilledInfo.total === 0 ||
    (halfFilledInfo.halfFilled / halfFilledInfo.total) <= 0.50;
  ...
  if (score >= 95) level = 'LazyMan';
  else if (score >= 88) level = 'Near-LazyMan';
  ...
  return {
    ...
    details: {
      ...
      pageNumbersFound: hasPageNumbers(dump),
      metadataFound: hasMetadata(dump),
      maxRepetition: maxRepetition(dump),
      titleVariance: titleLengthVariance(dump),
      halfFilledRatio: halfFilledInfo.total > 0
        ? (halfFilledInfo.halfFilled / halfFilledInfo.total).toFixed(2)
        : '0.00'
    }
  };
}
