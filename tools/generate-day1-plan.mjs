...
const analysisPath = path.resolve(repoRoot, 'docs/benchmarks/training/common_issues_analysis.json');
const analysis = JSON.parse(fs.readFileSync(analysisPath, 'utf8'));

const topIssue = analysis.commonIssues?.[0] || null;
const avgScoreNum = parseFloat(analysis.avgLazymanScore || '0');

const day1Plan = `# Day 1 修复计划
...
- 平均autoScore: ${analysis.avgDetails.autoScore}
- 距离LazyMan标准(95分)差距: ${(95 - avgScoreNum).toFixed(0)}分
...
\`\`\`
失败检查: ${topIssue?.check || 'N/A'}
影响范围: ${topIssue?.percentage || 'N/A'}的样本
当前半填充率: ${analysis.avgDetails.halfFilledRatio} (目标: ≤0.15)
\`\`\`
...
修复前：avgScore = ${analysis.avgLazymanScore}
修复后：avgScore ≥ 95
提升：+${(95 - avgScoreNum).toFixed(0)}分
...
const outPath = path.resolve(repoRoot, 'docs/benchmarks/training/day1_plan.md');
fs.writeFileSync(outPath, day1Plan, 'utf8');

console.log('✅ Day 1修复计划已生成: docs/benchmarks/training/day1_plan.md\n');
console.log(day1Plan);
